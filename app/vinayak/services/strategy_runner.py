...
class StrategyRunnerService:
    ...
    def run(self, candles: CandleBatch, config: StrategyConfig) -> StrategySignalBatch:
        strategy_name = str(config.strategy_name or '').strip().upper()
        strategy = self.strategy_registry.get(strategy_name)
        if strategy is None:
            supported = ', '.join(sorted(self.strategy_registry))
            raise ValueError(
                f'Unsupported strategy "{config.strategy_name}". Expected one of: {supported}.'
            )
