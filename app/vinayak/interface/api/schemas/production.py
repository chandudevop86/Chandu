...
_SUPPORTED_SIGNAL_STRATEGIES = {'BREAKOUT', 'DEMAND_SUPPLY', 'CONFIRMATION'}
...
class SignalRunRequest(BaseModel):
    ...
    def _validate_strategy(cls, value: str) -> str:
        ...
        if normalized not in _SUPPORTED_SIGNAL_STRATEGIES:
            supported = ', '.join(sorted(_SUPPORTED_SIGNAL_STRATEGIES))
            raise ValueError(f'Unsupported strategy. Expected one of: {supported}.')
        return normalized
